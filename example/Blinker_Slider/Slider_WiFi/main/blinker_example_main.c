...
BlinkerSlider slider1 = {.name = "slikey"};

int counter = 0;

void slider1_callback(int32_t value)
{
    BLINKER_LOG(TAG, "get slider value: %d", value);

    char value_back[10];
    sprintf(value_back, "%d", value);

    blinker_slider_config_t config = {
        .color = "0xFF",
        .value = value_back,
    };

    blinker_slider_print(&slider1, &config);
}
