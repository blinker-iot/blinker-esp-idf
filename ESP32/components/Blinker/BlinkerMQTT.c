...
static void http_serve(struct netconn *conn) {
    ...
    ESP_LOGI(TAG,"read from client : %d", err);
    if(err==ERR_OK) {
        netbuf_data(inbuf, (void**)&buf, &buflen);
        ESP_LOGI(TAG,"read from client : %d", buflen);
        if(buf) {
...
{
    BLINKER_LOG_ALL(TAG, "data from : %s", dataFrom_MQTT ? "ws":"mqtt");
    if (dataFrom_MQTT == BLINKER_MSG_FROM_WS)
    {
        ...
        char *_data;
        uint16_t _len = strlen(data) + 2;

        _data = (char*)malloc(_len*sizeof(char));
        strcpy(_data, data);
        strcat(_data, "\n");
